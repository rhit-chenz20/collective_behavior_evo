...
plot_summary_multi_by_psi_ticks <- function(summary_stat_df,
  ...
  # Figure out which regimes to plot
  if (is.null(regimes)) {
  ...
  # ---- NEW: loop over regimes and write to separate subfolders ----
  for (reg_lab in regimes) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in shift_keys) {
        ...
        mtext(paste0("Summary: dist_norm × Va0_sqrt over ticks | reg = ", reg_lab,
                     " | n = ", n_val, " | ", title_suffix),
              outer = TRUE, cex = 1.1, line = 1.2)
...
# ---- Run ----
# Auto-detects regs from summary_stat_df$reg; or pass regimes = c("ave","fit")
