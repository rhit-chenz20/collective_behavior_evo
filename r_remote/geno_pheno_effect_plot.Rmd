...
extract_geno_metadata <- function(path) {
  # Extract job_id (x), job_rep (i), psi (y), burnin_id (z)
...
psi_vals <- sort(unique(gen10100_df$psi))
...
for (bid in burnin_ids) {
  ...
  psi_in_this_burnin <- sort(unique(df_bid$psi))
  ...
  for (psi in psi_in_this_burnin) {
    ...
    legend_labels <- c(legend_labels, paste0("psi=", psi, " (n=", n_reps, ")"))
  }
...
library(viridis)
palette <- viridis(length(unique(genotype_df$psi)), option = "C")
...
# Unique psi values
psi_vals <- sort(unique(genotype_df$psi))

# Initialize matrix for each psi
...
for (i in seq_along(psi_vals)) {
  psi <- psi_vals[i]
  df_psi <- subset(genotype_df, psi == psi & generation == 10100)
...
# Color palette (Zissou1 style)
library(viridis)
palette <- viridis(length(unique(phenotype_df$psi)), option = "C")
...
# Unique psi values
psi_vals <- sort(unique(phenotype_df$psi))
...
for (i in seq_along(psi_vals)) {
  psi <- psi_vals[i]
  df_psi <- subset(phenotype_df, psi == psi & generation == 10100)
...
reordered_list <- lapply(df_list, function(df) {
  ...
  colnames(meta_cols) <- c("job_id", "psi", "burnin_id", "file_id")
...
for (i in seq_along(psi_vals)) {
  psi <- psi_vals[i]
  df_psi <- subset(effect_df, psi == psi)
...
# ---- PLOT ----
plot(NULL, xlim = c(-3, 3), ylim = c(0, 1.0), xlab = "Effect Size", ylab = "Density",
     main = "Effect Size Distribution Averaged by psi\n(Rep = burnin × job_id)")
...
# Save plot
file_name <- sprintf("%s/effect_dist_by_psi_combined.png", outdir)
dev.copy(png, filename = file_name, width = 800, height = 600)
dev.off()
...
# Create one plot for all psi values with burnin_id pooled
plot_matrix <- matrix(0, nrow = length(psi_vals), ncol = length(bin_centers))
rep_counts <- integer(length(psi_vals))

for (i in seq_along(psi_vals)) {
  psi <- psi_vals[i]
  df_psi <- subset(effect_df, generation == "10100" & psi == psi)
...
# # Filter the dataset
# df_10100 <- subset(df_10100, psi %in% psi_keep)

# Prepare color palette
psi_vals <- sort(unique(df_10100$psi))
palette <- viridis(length(psi_vals), option = "C")
...
# Loop by psi and gather all mutation info
all_data <- list()
for (i in seq_along(psi_vals)) {
  psi <- psi_vals[i]
  df_psi <- subset(df_10100, psi == psi)
...
for (i in seq_along(psi_vals)) {
  psi <- psi_vals[i]
  sub_df <- subset(plot_df, psi == psi)
  points(sub_df$effect, sub_df$allele_freq, col = palette[i], pch = 16, cex = 0.6)
}

legend("topright", legend = paste0("psi=", psi_vals), col = palette, pch = 16, cex = 0.8)
...
# Define the psi values you want to keep
psi_keep <- c(0.9, 0.8, 0.7, 0.2, 0.1, 0.0, -0.2, -0.5, -0.7, -0.8, -0.9)
...
for (bid in unique(df_10100$burnin_id)) {
  ...
  for (i in seq_along(psi_vals)) {
    ...
    if (nrow(df_psi) == 0) next  # <-- SKIP if no data for this psi
...
# Set psi values to keep
psi_keep <- c(0.9, 0.8, 0.6, 0.3, 0.0, -0.3, -0.6, -0.8, -0.9)

# Filter generation 10100 and psi of interest
df_10100 <- subset(effect_df, generation == "10100" & psi %in% psi_keep)
...
legend("topright", legend = paste0("psi=", psi_vals[valid_idx], " (n=", rep_counts[valid_idx]/1000, ")"),
...
# Preview expected structure: seg_site, psi, rep, burnin_id
stopifnot(all(c("seg_site", "psi", "rep") %in% names(data_df)))

# ---- PLOT ----
# Set up plot
plot(NULL, xlim = range(data_df$psi), ylim = range(data_df$seg_site),
     xlab = "psi", ylab = "number of segregating sites", main = "number of segregating sites vs psi across replicates")
...
for (i in seq_along(burnin_ids)) {
  ...
  points(df_b$psi, df_b$seg_site, col = colors[i], pch = 16)
}
...
psi_vals <- sort(unique(mut_df$psi))
palette <- viridis(length(psi_vals), option = "C")
...
plot(NULL, xlim = c(NA, NA), ylim = c(0, NA),
     ...
     main = "Effect Size Histograms for All psi Values")
...
plot(NULL, xlim = c(x_min, x_max), ylim = c(0, y_max * 1.05),
     xlab = "Effect size", ylab = "Density",
     main = "Effect Size for Unique Mutations by psi")
