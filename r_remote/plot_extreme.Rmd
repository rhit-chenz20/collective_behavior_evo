...
base_data_folder <- "../data/n_group_extreme"
base_plot_folder <- "../r_plots/n_group_extreme"
...
num_bins <- 160
bin_breaks <- seq(-80, 80, length.out = num_bins + 1)
...
avg_curve_for_subset <- function(df_block) {
  ...
  df_reg_all <- subset(genotype_df, generation == target_gen)
  ...
  for (n_val in sort(unique(df_reg_all$n))) {
    ...
    out_path <- file.path(
      out_folder,
      paste0("/genotype_hist_avg_n_", n_val, "_gen_", target_gen, ".png")
    )
...
plot_genotype_multi_by_psi <- function(genotype_df,
  ...
  if (!("sz_orig" %in% names(genotype_df))) {
    ...
    df_reg <- genotype_df
    if (!dir.exists(file.path(base_plot_folder,out_subfolder))) dir.create(file.path(base_plot_folder,out_subfolder), recursive = TRUE)
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in shift_keys) {
        ...
        out_name <- paste0(
          "genotype_multi_by_psi_n_", n_val, "_",
          gsub("[ =]", "", title_suffix), ".png"
        )
        png(file.path(base_plot_folder,out_subfolder, out_name), width = png_w, height = png_h)
...
avg_curve_for_subset_pheno <- function(df_block, bin_breaks, bin_mids) {
  ...
  df_reg <- subset(phenotype_df, generation == target_gen)
  ...
  for (n_val in sort(unique(df_reg$n))) {
    ...
    out_path <- file.path(out_dir, paste0("phenotype_hist_n_", n_val, "_gen_", target_gen, ".png"))
    png(out_path, width = png_w, height = png_h)
    op <- par(mfrow = c(nrow, ncol), mar = c(4, 4, 3.5, 1) + 0.1)
  ...
  if (!("reg" %in% names(phenotype_df))) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in shift_keys) {
        ...
        out_name <- paste0("phenotype_multi_by_psi_reg_n_", n_val, "_", gsub("[ =]", "", title_suffix), ".png")
...
# One PDF per value of `split_var` (default: psi)
...
plot_seg_site_by_tick <- function(df,
  ...
  df$n    <- suppressWarnings(as.integer(df$n))
    ...
    df_reg <- subset(df, tick %in% tick_window)
    if (!nrow(df_reg)) next

    # Make regime-specific folder
    out_dir <- file.path(base_plot_folder, out_root)
    if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

    # Unique split values (e.g., psi or dist_norm)
    if (!split_var %in% names(df_reg)) {
      warning("split_var '", split_var, "' not found; skipping")
      next
    }
...
plot_seg_site_by_tick(summary_stat_df,
                      out_root = "seg_site_overtime_9999:10005")
...
# Plots dist_norm * Va0_sqrt vs ticks, per psi
...
.compute_series <- function(df_panel, tick_seq) {
  ...
  needed <- c("dist_norm","Va0_sqrt","tick","rep","z_mean")
  ...
  Va0_sqrt_num  <- suppressWarnings(as.numeric(df_sub$Va0_sqrt))
  ...
  # Base y from dist_norm * Va0_sqrt
  y <- dist_norm_num * Va0_sqrt_num
  ...
  if ("optimum" %in% names(df_sub)) {
    ...
    if (any(rows_opt)) {
      ...
      y[idx_9999] <- abs(z_mean_num[idx_9999] - rep_opt[idx_9999])
    }
  }
  ...
  has_sz_orig <- "sz_orig" %in% names(summary_stat_df)
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in shift_keys) {
        ...
        out_name <- paste0("summary_multi_by_psi_ticks_reg_n_", n_val, "_", gsub("[ =]", "", title_suffix), ".pdf")
        ...
        mtext(paste0("Summary: dist_norm * Va0_sqrt over ticks (9999 uses |z_mean - opt@10000|) | n = ", n_val, " | ", title_suffix),
...
```{r}
# ---- Run ----
plot_summary_multi_by_psi_ticks(summary_stat_df,
                                out_subfolder = "distanct_from_opt_overtime")

```

```{r}
# --------- 1) Collect decay metrics per (reg, shift_key, n, psi) ----------
...
compute_decay_table <- function(df, tick_seq) {
  stopifnot(all(c("n","psi","tick","reg","shift_size") %in% names(df)))
  ...
  for (reg_val in sort(unique(df$reg))) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in shift_keys) {
        ...
        for (psi_val in sort(unique(df_group$psi))) {
          df_p <- subset(df_group, psi == psi_val & tick %in% tick_seq)
          s <- .compute_series(df_p, tick_seq)
...
# one forest plot per shift_key
for (sk in sort(unique(decay_tbl$shift_key))) {
  df_sk <- subset(decay_tbl, shift_key == sk)
  if (!nrow(df_sk)) next

  out_name <- paste0("decay_forest_half_life_", gsub("[ =]", "", sk), ".pdf")
  plot_decay_forest(df_sk, out_name = out_name)
}
...
gen_filter  <- "10045"   # encoded in filenames to keep
  ...
  avg_curve_for_subset <- function(df_block, breaks, num_bins) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sh in sort(unique(df_n$shift_size))) {
        ...
        # open PNG
        fname <- paste0("ind_effect_dists_reg_n_", n_val, "_", file_shift, ".png")
...
plot_ind_effect_dists_by_psi(
  ...
  out_subfolder = paste0("ind_effect_dists_by_psi_gen", gen_filter)
)
```
...
plot_mut_counts_over_psi <- function(mut_count_df,
  ...
  mean_col <- "steelblue4"
    ...
    d_reg <- subset(mut_count_df, generation == gen_target)
    if (nrow(d_reg) == 0) {
      warning("No rows at generation ", gen_target, " for in mut_count_df.")
      next
    }
    ...
    for (n_val in sort(unique(d_reg$n))) {
      ...
      for (sh in sort(unique(d_n$shift_size))) {
        ...
        fname <- paste0("mut_count_over_psi_reg_n_", n_val, "_", file_shift, "_gen", gen_target, ".png")
        png(file.path(out_dir, fname), width = 1000, height = 700)
        par(mar = c(4.5, 4.5, 3.5, 1) + 0.1)
        ...
        plot(NA,
             ...
             main = paste0("Mutations vs psi | n = ", n_val, " | ", title_shift,
                           " | gen = ", gen_target),
             xaxt = "n")
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sh in sort(unique(df_n$shift_size))) {
        ...
        for (g in gens) {
          ...
          # File + device
          fname <- paste0("mut_avg_hist_reg_n_", n_val, "_", file_shift, "_gen", g, ".png")
          png(file.path(out_dir, fname), width = 1000, height = 700)
          par(mar = c(4.5, 4.5, 3.5, 1) + 0.1)
...
plot_mut_hist_two_gens(
  ...
  out_subfolder = "mut_effect_pop_level"
)
