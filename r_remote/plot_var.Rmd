...
base_data_folder <- "../data/variance_geno"
base_plot_folder <- "../r_plots/new_var_plots"
target_beta <- c(0.0, 0.01, 0.03, 0.05, 0.3, 0.5, 0.7, 0.9)
regimes <- c("best", "inv", "ave")  # "ave" = average, "fit" = fitness-based
```
...
# Matches: genotype/n_<n>_beta_<beta>_sz_<sz>_reg_<ave|fit>_<rep>.tsv
extract_geno_metadata <- function(path) {
  pat <- "genotype/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|best|ave)_(\\d+)\\.tsv"
...
num_bins <- 3000
bin_breaks <- seq(-200, 1500, length.out = num_bins + 1)
...
for (reg_lab in regimes) {
  ...
  for (n_val in sort(unique(df_reg_all$n))) {
    ...
    out_folder <- file.path(base_plot_folder, paste0("genotype_", target_gen))
...
plot_genotype_multi_by_beta <- function(genotype_df,
  ...
  for (reg_lab in regimes) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      all_beta <- sort(unique(df_n$beta))
...
# Matches: phenotype/n_<n>_beta_<beta>_sz_<sz>_reg_<ave|fit>_<rep>.tsv
extract_pheno_metadata <- function(path) {
  m <- regexec("phenotype/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|inv|best)_(\\d+)\\.tsv", path)
...
# Define histogram bins
num_bins <- 2100
bin_breaks <- seq(-100, 2000, length.out = num_bins + 1)
bin_mids <- 0.5 * (head(bin_breaks, -1) + tail(bin_breaks, -1))
...
# load group variance data
```{r}
# Top-level folder
folder <- file.path(base_data_folder, "vars")
...
# Extract metadata from path like: vars/n_<n>_beta_<beta>_sz_<sz>_reg_(inv|best|ave)_<rep>.tsv
extract_var_metadata <- function(path) {
  pat <- "vars/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|best|ave)_(\\d+)\\.tsv"
  m <- regexec(pat, path); parts <- regmatches(path, m)[[1]]
  if (length(parts) != 6) {
    warning("Could not extract metadata from path: ", path,
            " ; matches: ", paste(parts, collapse = ", "))
    return(data.frame())
  }
  data.frame(
    n     = as.integer(parts[2]),
...
# Helper: wide (X*) -> long (feature,value) per file, then attach meta
# Keeps id columns if present (e.g., generation/tick), and optionally filters to a generation.
to_long_vars <- function(df, meta, file, opt_gen = NA_integer_) {
  # Keep a couple of possible id columns if present
  id_cols <- intersect(c("generation","tick"), names(df))

  # Optional filter to a specific generation (set opt_gen if you want it)
  if (is.finite(opt_gen) && "generation" %in% names(df)) {
    df <- df[df$generation == opt_gen, , drop = FALSE]
    if (!nrow(df)) return(NULL)
  }

  # Value columns assumed to be X*
  x_cols <- grep("^X", names(df), value = TRUE)
  if (!length(x_cols)) {
    warning("No X* columns in file: ", file)
    return(NULL)
  }

  # Build long table (base R, memory-safe)
  mat <- as.matrix(df[, x_cols, drop = FALSE])
  nr  <- nrow(df); nc <- ncol(mat)

  long <- data.frame(
    # replicate id cols per feature
    if (length(id_cols)) df[id_cols][rep(seq_len(nr), times = nc), , drop = FALSE],
    # metadata (replicated for all rows in this file)
    rep     = rep(meta$rep,     nr * nc),
    beta    = rep(meta$beta,    nr * nc),
    n       = rep(meta$n,       nr * nc),
    reg     = rep(meta$reg,     nr * nc),
    sz_orig = rep(meta$sz,      nr * nc),
    shift_size = rep(meta$sz,   nr * nc),
    file_id = rep(basename(file), nr * nc),
    # long keys + values
    feature = as.integer(sub("^X", "", rep(colnames(mat), each = nr))),
    value   = suppressWarnings(as.numeric(as.vector(t(mat)))),
    stringsAsFactors = FALSE
  )
  long
}

# ------------ Load all files -> ONE long table ------------
opt_gen <- 9999   # set to 10001L if you want to keep only that generation
pieces <- lapply(file_list, function(file) {
  df <- tryCatch(
  ...
  if (is.null(df)) {
    warning("Failed to read vars file: ", file)
    return(NULL)
  }
  meta <- extract_var_metadata(file)
  if (nrow(meta) == 0) return(NULL)
  to_long_vars(df, meta, file, opt_gen = opt_gen)
})

pieces <- Filter(Negate(is.null), pieces)
if (length(pieces)) {
  group_var_long <- do.call(rbind, pieces)
} else {
  group_var_long <- data.frame()
}
...
# Assuming you want to focus on a specific generation (e.g., 10100)
target_gen <- 9999
df <- subset(group_var_df, generation == target_gen)

# Define histogram bins
num_bins <- 400
bin_breaks <- seq(-1, 400, length.out = num_bins + 1)
bin_mids <- 0.5 * (head(bin_breaks, -1) + tail(bin_breaks, -1))
...
print(max(group_var_long$value, na.rm=TRUE))
print(min(group_var_long$value, na.rm=TRUE))
```

## plot average vars for each beta
```{r}
.open_pdf <- function(path, width, height, family = "sans") {
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  if (capabilities("cairo")) {
    grDevices::cairo_pdf(file = path, width = width, height = height, family = family)
  } else {
    grDevices::pdf(file = path, width = width, height = height, family = family, useDingbats = FALSE)
  }
}


plot_group_var_hists_long <- function(group_var_long,
                                      regimes,
                                      target_gen,
                                      include_pooled_sz0 = FALSE,
                                      ref_sd = sqrt(2000),
                                      out_subfolder = "group_var_from_long_pdf",
                                      panel_w = 4.5, panel_h = 3.5,
                                      exclude_out_range = FALSE) {
  out_folder <- file.path(base_plot_folder, out_subfolder)
  if (!dir.exists(out_folder)) dir.create(out_folder, recursive = TRUE)

  time_col <- if ("generation" %in% names(group_var_long)) "generation" else "tick"

  avg_curve_for_subset_long <- function(df_block) {
    if (!nrow(df_block)) return(rep(NA_real_, length(bin_mids)))
    vals_all <- suppressWarnings(as.numeric(df_block$value))
    if (!any(is.finite(vals_all))) return(rep(NA_real_, length(bin_mids)))
    reps <- if ("rep" %in% names(df_block)) sort(unique(df_block$rep)) else NA_integer_
    counts_mat <- matrix(NA_real_, nrow = length(reps), ncol = length(bin_mids))
    for (i in seq_along(reps)) {
      vals <- if (!is.na(reps[i])) suppressWarnings(as.numeric(df_block$value[df_block$rep == reps[i]])) else vals_all
      vals <- vals[is.finite(vals)]
      if (!length(vals)) next
      if (exclude_out_range) { 
        vals=vals[vals >= min(bin_breaks) & vals <= max(bin_breaks) ]
      }
      
      h <- hist(vals, breaks = bin_breaks, plot = FALSE)
      tot <- sum(h$counts); if (tot <= 0) next
      counts_mat[i, ] <- h$counts / tot
    }
    good <- which(rowSums(is.finite(counts_mat)) > 0)
    if (!length(good)) return(rep(NA_real_, length(bin_mids)))
    colMeans(counts_mat[good, , drop = FALSE], na.rm = TRUE)
  }

  for (reg_lab in regimes) {
    # >>> FIX: use base indexing, not .data <<<
    keep <- (group_var_long[[time_col]] == target_gen) & (group_var_long$reg == reg_lab)
    df_reg_all <- group_var_long[keep, , drop = FALSE]
    if (!nrow(df_reg_all)) next

    for (n_val in sort(unique(df_reg_all$n))) {
      df_n <- df_reg_all[df_reg_all$n == n_val, , drop = FALSE]
      if (!nrow(df_n)) next

      has_sz0 <- "sz_orig" %in% names(df_n) && any(df_n$sz_orig == 0, na.rm = TRUE)
      nonzero <- sort(unique(df_n$shift_size[if ("sz_orig" %in% names(df_n)) df_n$sz_orig != 0 else TRUE]))
      nonzero <- nonzero[is.finite(nonzero)]
      panel_keys <- c(if (include_pooled_sz0 && has_sz0) "pooled_sz0" else character(0),
                      if (length(nonzero)) paste0("shift=", nonzero) else character(0))
      if (!length(panel_keys)) next
      ...
      for (pk in panel_keys) {
        df_panel <- if (pk == "pooled_sz0") {
          df_n[!is.na(df_n$sz_orig) & df_n$sz_orig == 0, , drop = FALSE]
        } else {
          sh <- as.numeric(sub("shift=", "", pk))
          df_n[df_n$shift_size == sh & (if ("sz_orig" %in% names(df_n)) df_n$sz_orig != 0 else TRUE), , drop = FALSE]
        }
        if (exclude_out_range) {
          df_panel <- df_panel[df_panel$value >= min(bin_breaks) & df_panel$value <= max(bin_breaks), , drop = FALSE]
        }

        if (!nrow(df_panel)) next
        for (beta_val in beta_all) {
          avg_counts <- avg_curve_for_subset_long(df_panel[df_panel$beta == beta_val, , drop = FALSE])
          if (any(is.finite(avg_counts))) max_y <- max(max_y, avg_counts, na.rm = TRUE)
        }
      }
      if (!is.finite(max_y) || max_y <= 0) max_y <- 1
      ...
      out_path <- file.path(out_folder, sprintf("group_var_hist_avg_reg_%s_n_%s_gen_%s_%s-%s.pdf",
                                                reg_lab, n_val, target_gen, min(bin_breaks), max(bin_breaks)))
      .open_pdf(out_path, width = ncol * panel_w, height = nrow * panel_h)
      op <- par(mfrow = c(nrow, ncol), mar = c(4, 4, 3.5, 1) + 0.1, family = "sans")

      for (pk in panel_keys) {
        if (pk == "pooled_sz0") {
          df_panel <- df_n[!is.na(df_n$sz_orig) & df_n$sz_orig == 0, , drop = FALSE]
          sh_current <- NA_real_
        } else {
          sh_current <- as.numeric(sub("shift=", "", pk))
          df_panel <- df_n[df_n$shift_size == sh_current & (if ("sz_orig" %in% names(df_n)) df_n$sz_orig != 0 else TRUE),
                           , drop = FALSE]
        }

        panel_title <- if (is.na(sh_current)) {
          sprintf("reg=%s | n=%s | sz=0 pooled", reg_lab, n_val)
        } else {
          sprintf("reg=%s | n=%s | shift=%s", reg_lab, n_val, sh_current)
        }

        plot(NULL, xlim = range(bin_breaks), ylim = c(0, max_y),
             xlab = "Group variance", ylab = "Proportion", main = panel_title)

        if (is.finite(sh_current)) curve(dnorm(x, mean = sh_current, sd = ref_sd), add = TRUE, col = "black", lty = 2, lwd = 2)
        curve(dnorm(x, mean = 0, sd = ref_sd), add = TRUE, col = "black", lty = 3, lwd = 2)
        ...
        if (length(present_betas)) {
          legend("topright", legend = paste0("beta=", present_betas),
                 col = colors[as.character(present_betas)], lty = 1, lwd = 2, cex = 0.85, bg = "white")
        }
      }
...
#   data/n_<n>/beta_<beta>/sz_<shift_size>/_reg_<ave|fit>_<rep>.tsv
extract_sum_metadata <- function(path) {
  rx <- "data/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|best|inv)_(\\d+)\\.tsv"
  m  <- regexec(rx, path)
...
# Filter usable rows
decay_tbl <- subset(decay_tbl, is.finite(t_half))

# --------- 2A) Forest / dot plot of half-life by parameter set ----------
plot_decay_forest <- function(decay_tbl,
                              by = c("reg","shift_key"),  # facet keys
                              x_metric = c("t_half","k")[1],
                              out_name = "decay_forest.pdf") {
  xlab <- if (x_metric == "t_half") "Half-life (ticks)" else "Rate k (1/ticks)"
  x    <- decay_tbl[[x_metric]]
  ord  <- order(x, decreasing = FALSE)

  # Label each row compactly
  row_lab <- with(decay_tbl,
                  paste0("beta=", beta, " | n=", n, " | ", shift_key, " | ", reg))

  # Compute 95% CI for k if available; transform to t_half CI if needed
  x_lo <- x_hi <- rep(NA_real_, length(x))
  if (x_metric == "k" && "k_se" %in% names(decay_tbl) && any(is.finite(decay_tbl$k_se))) {
    x_lo <- decay_tbl$k - 1.96*decay_tbl$k_se
    x_hi <- decay_tbl$k + 1.96*decay_tbl$k_se
  } else if (x_metric == "t_half" && "k_se" %in% names(decay_tbl) && any(is.finite(decay_tbl$k_se))) {
    # delta method on t_half = log(2)/k → se(t_half) ≈ | -log(2)/k^2 | * se(k)
    se_t <- abs(log(2) / (decay_tbl$k^2)) * decay_tbl$k_se
    x_lo <- x - 1.96*se_t
    x_hi <- x + 1.96*se_t
  }

  pdf(file.path(base_plot_folder, out_name), width = 10, height = 12)
  op <- par(mar = c(4, 12, 3, 1) + 0.1)

  dotchart(x[ord],
           labels = row_lab[ord],
           xlab = xlab, main = paste0("Exponential decay comparison (", x_metric, ")"),
           pch = 19, cex = 0.8)

  # Add CIs if we have them
  has_ci <- is.finite(x_lo[ord]) & is.finite(x_hi[ord])
  if (any(has_ci)) {
    y <- seq_along(x[ord])[has_ci]
    segments(x0 = x_lo[ord][has_ci], y0 = y, x1 = x_hi[ord][has_ci], y1 = y, lwd = 2)
  }

  par(op); dev.off()
}

for (sk in sort(unique(decay_tbl$shift_key))) {
  for (reg_val in sort(unique(decay_tbl$reg))) {
    df_sub <- subset(decay_tbl, shift_key == sk & reg == reg_val)
    if (!nrow(df_sub)) next

    out_name <- paste0("decay_forest_half_life_", reg_val, "_", gsub("[ =]", "", sk), ".pdf")
    plot_decay_forest(df_sub, out_name = out_name)
  }
}
...
# Filename: ind/n_{n}_beta_{beta}_sz_{shift}_reg_{ave|fit}_{rep}_gen{gen_filter}.tsv
extract_effect_metadata <- function(path) {
  rx <- paste0("ind/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|fit)_(\\d+)_gen", gen_filter, "\\.tsv")
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 6) {
...
#   mut/n_{n}_beta_{beta}_sz_{shift}_reg_{ave|fit}_{rep}.tsv
extract_mut_metadata <- function(path) {
  rx <- "mut/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|fit)_(\\d+)\\.tsv"
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 6) {
    warning("Could not extract metadata from path: ", path, " | parts: ", paste(p, collapse=", "))
...
# --- Config (bins) ---
num_bins   <- 1000
bin_breaks <- seq(-100, 1000, length.out = num_bins + 1)
...
# mut/n_{n}_beta_{beta}_sz_{shift}_reg_{ave|fit}_{rep}.tsv
extract_mut_metadata <- function(path) {
  rx <- "mut/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|best|inv)_(\\d+)\\.tsv"
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
...
plot_mut_hist_two_gens(
  mut_hist_mean,
  gens = c(9999, 10005, 10015),
  out_subfolder = "mut_effect_pop",
  regimes = regimes  # e.g., c("ave","fit")
)
