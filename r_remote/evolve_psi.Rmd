...
base_data_folder <- "../data/evolve_psi"
base_plot_folder <- "../r_plots/evolve_psi"
target_psi <- c(0.0)
...
extract_geno_metadata <- function(path) {
  pat <- "genotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|fit|ext)_(\\d+)\\.tsv"
...
extract_mut_metadata <- function(path) {
  rx <- "mut/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(ave|fit|ext)_(\\d+)\\.tsv"
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 6) {
    warning("Could not extract metadata from path: ", path, " | parts: ", paste(p, collapse=", "))
