...
target_alphas <- c(-0.9, -0.5, -0.3, 0.0, 0.3, 0.5, 0.9)
```
...
for (n_val in sort(unique(genotype_df$n))) {
  df_n <- subset(genotype_df, generation == 10099 & n == n_val & alpha %in% target_alphas)
...
for (n_val in sort(unique(phenotype_df$n))) {
  df_n <- subset(phenotype_df, generation == 10099 & n == n_val & alpha %in% target_alphas)
...
extract_mut_metadata <- function(path) {
  matches <- regexec("mut_(\\d+)_([1-3])/n_(\\d+)/rec_N_1000_alpha_(-?[0-9.]+)_pop_(\\d+)_1\\.tsv", path)
  parts <- regmatches(path, matches)[[1]]
