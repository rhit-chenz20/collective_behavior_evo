...
base_data_folder <- "../data/n_group_maxfit_data"
...
for (n_val in sort(unique(genotype_df$n))) {
  df_n <- subset(genotype_df, generation == 10099 & n == n_val & psi %in% target_psis)
  ...
  for (bid in burnin_ids) {
    ...
    plot(NULL, xlim = c(-40, 40), ylim = c(0, 0.3),
         xlab = "Geenotype Value", ylab = "Proportion",
         main = paste("Geenotype Histogram (per rep) | n =", n_val, "| burnin_id =", bid))
 ...
 curve(dnorm(x, mean = 0, sd = sqrt(2000) * diff(breaks)[1]), 
    ...
    for (psi1 in psi_in_this_bid) {
      df_psi <- subset(df_bid, psi == psi1)
      ...
      for (r in reps) {
        ...
        col_transparent <- adjustcolor(colors[as.character(psi1)], alpha.f = 0.4)
        lines(bin_mids, prop, type = "l", col = col_transparent, lwd = 1)
      }

      legend_labels <- c(legend_labels, paste0("psi=", psi1))
    }
...
for (n_val in sort(unique(genotype_df$n))) {
 ...
 curve(dnorm(x, mean = 0, sd = sqrt(2000) * diff(breaks)[1]), 
  ...
  for (psi1 in psi_in_this_n) {
    df_psi <- subset(df_n, psi == psi1)
    reps <- unique(df_psi$job_id)
...
for (psi1 in psi_vals) {
  ...
  # Plot average line
  lines(bin_mids, avg_counts, col = colors[as.character(psi1)], lwd = 2)

  legend_labels <- c(legend_labels, paste0("psi=", psi1, " (n=", nrow(df_psi), ")"))
}
  ...
  for (psi1 in psi_vals) {
    ...
    for (r in reps) {
      ...
      curve(ecdf_line(x), from = min(ran), to = max(ran), add = TRUE,
            col = adjustcolor(colors[as.character(psi1)], alpha.f = 0.4),
            lwd = 1)
    }
...
for (n_val in sort(unique(phenotype_df$n))) {
  df_n <- subset(phenotype_df, generation == 10099 & n == n_val & psi %in% target_psis)
...
## plot average phenotype data across bid
```{r}
psi_vals <- sort(target_psis)
colors <- hcl.colors(length(psi_vals), palette = "Zissou 1")
names(colors) <- as.character(psi_vals)
...
for (n_val in sort(unique(phenotype_df$n))) {
  ...
  png(file.path(output_dir, paste0("phenotype_hist_avg_n_", n_val, ".png")),
      width = 1000, height = 700)

  plot(NULL, xlim = c(min(breaks), max(breaks)), ylim = c(0, 0.3),
       xlab = "Phenotype Value", ylab = "Proportion",
       main = paste("Phenotype Histogram | n =", n_val))
...
```{r}
# Set the top-level folder
folder <- paste0(base_data_folder, "/mut")
...
extract_mut_metadata <- function(path) {
  matches <- regexec("mut_(\\d+)_([1-3])/n_(\\d+)/rec_N_1000_psi_(-?[0-9.]+)_pop_(\\d+)_1\\.tsv", path)
  parts <- regmatches(path, matches)[[1]]
