...
base_data_folder <- "../data/n_group_base_dir"
base_plot_folder <- "../r_plots/n_group_base_dir"
...
extract_geno_metadata <- function(path) {
  matches <- regexec("genotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_(\\d+)\\.tsv", path)
...
genotype_df <- do.call(rbind, lapply(file_list, function(file) {
  ...
  df$psi <- meta$psi
  df$n   <- meta$n
  df$sz_orig    <- meta$sz           # <- preserve original sz (0 or not)
  df$shift_size <- if (meta$sz == 0L) get_shift_from_data(file, base_data_folder, 10001L) else meta$sz
...
```{r}
# Assumes genotype_df already has: generation, n, psi, rep, shift_size, sz_orig, X* columns
# Also assumes you have: target_gen, num_bins, bin_breaks, bin_mids, base_plot_folder

avg_curve_for_subset <- function(df_block) {
  # df_block is already filtered to a single (panel × psi) subset at target_gen
...
for (n_val in sort(unique(genotype_df$n))) {
  ...
  # Colors for psi (consistent across all panels in this n)
  ...
  png_w <- 900 * ncol
  png_h <- 700 * nrow
  out_path <- file.path(base_plot_folder, paste0("genotype_hist_avg_n_", n_val, "_gen_", target_gen,".png"))
  ...
  for (pk in panel_keys) {
    ...
    panel_title <- if (pk == "pooled_sz0") {
      paste0("n = ", n_val, " | sz=0 pooled (data-driven)")
    } else {
      paste0("n = ", n_val, " | ", pk)
    }
...
# and per-psi pooled shift_size in titles
# ---------------------------------------------

# You already have these (or define them as below):
# num_bins <- 100
# bin_breaks <- seq(-50, 50, length.out = num_bins + 1)
# gen_seq <- 10000:10005
...
plot_genotype_multi_by_psi <- function(genotype_df,
  ...
  for (n_val in sort(unique(genotype_df$n))) {
    df_n <- subset(genotype_df, n == n_val)
    if (nrow(df_n) == 0) next
    ...
    for (sk in shift_keys) {
      ...
      out_name <- paste0("genotype_multi_by_psi_n_", n_val, "_", gsub("[ =]", "", title_suffix), ".png")
      ...
      mtext(paste0("Genotype distributions (gen-specific bins) | n = ", n_val, " | ", title_suffix),
...
extract_pheno_metadata <- function(path) {
  m <- regexec("phenotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_(\\d+)\\.tsv", path)
...
for (n_val in sort(unique(phenotype_df$n))) {
  ...
  # Consistent psi colors across this n
  psi_all <- sort(unique(df_n$psi))
  ...
  out_path <- file.path(base_plot_folder, paste0("phenotype_hist_avg_n_", n_val, "_gen_", target_gen,".png"))
  png(out_path, width = png_w, height = png_h)
  op <- par(mfrow = c(nrow, ncol), mar = c(4, 4, 3.5, 1) + 0.1)
...
```{r}
# ---------------------------------------------
# Multi-panel by psi (PHENOTYPE) with gen-specific bins
# and per-psi pooled shift_size in titles
# ---------------------------------------------
# Prereqs in your env:
#   phenotype_df  (needs columns: generation, n, psi, rep, shift_size, sz_orig, X*)
#   base_plot_folder
#   num_bins, bin_breaks  (used to derive a constant bin width)
#   gen_seq <- 10000:10005   # generations to overlay
...
plot_phenotype_multi_by_psi <- function(phenotype_df,
  ...
  for (n_val in sort(unique(phenotype_df$n))) {
    df_n <- subset(phenotype_df, n == n_val)
    if (nrow(df_n) == 0) next
    ...
    for (sk in shift_keys) {
      ...
      # Pass 1: find common y-limit and x-range across all psi & gens
      max_y <- 0
      x_min <- Inf; x_max <- -Inf
      for (psi_val in all_psis) {
        df_p <- subset(df_group, psi == psi_val)
      ...
      out_name <- paste0("phenotype_multi_by_psi_n_", n_val, "_", gsub("[ =]", "", title_suffix), ".png")
      png(file.path(out_dir, out_name), width = png_w, height = png_h)
      op <- par(mfrow = c(nrow, ncol), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 3.5, 0))
      ...
      mtext(paste0("Phenotype distributions (gen-specific bins) | n = ", n_val, " | ", title_suffix),
            outer = TRUE, cex = 1.1, line = 1.2)
...
#   data/n_{n}_psi_{psi}_sz_{shift_size}_{rep}.tsv
extract_sum_metadata <- function(path) {
  rx <- "data/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_([0-9]+)\\.tsv"
  m  <- regexec(rx, path)
...
# Filename: ind/n_{n}_psi_{psi}_sz_{shift}_{rep}_gen{gen_filter}.tsv
extract_effect_metadata <- function(path) {
  rx <- paste0("ind/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_([0-9]+)_gen", gen_filter, "\\.tsv")
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 5) {
...
effects_rows <- lapply(df_list, function(df) {
  ...
  core <- c("generation","individual_id","rep","psi","n","shift_size","file_id")
  tok_idx <- which(!(names(df) %in% core))

  eff_list <- if (!length(tok_idx)) {
    replicate(nrow(df), numeric(0), simplify = FALSE)
  } else {
    # Apply row-wise over token columns; ensure list output
...
plot_ind_effect_dists_by_psi <- function(effect_df_wide,
  ...
  for (n_val in sort(unique(effect_df_wide$n))) {
    df_n <- subset(effect_df_wide, n == n_val)
    if (nrow(df_n) == 0) next
    ...
    for (sh in sort(unique(df_n$shift_size))) {
      ...
      # all psi values in this (n, shift) group
      ...
      # open PNG
      fname <- paste0("ind_effect_dists_n_", n_val, "_", file_shift, ".png")
      ...
      plot(NULL,
           ...
           main = paste0("Effect distributions | n = ", n_val, " | ", title_shift))
...
# ---- Run it ----
plot_ind_effect_dists_by_psi(effect_df_wide,
                             num_bins = 40,
...
# Filename pattern: mut/n_{n}_psi_{psi}_sz_{shift}_{rep}.tsv
extract_mut_metadata <- function(path) {
  rx <- "mut/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_([0-9]+)\\.tsv"
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 5) {
    warning("Could not extract metadata from path: ", path, " | parts: ", paste(p, collapse=", "))
...
.last_integer_token_idx <- function(tokens) {
  ...
  # Drop empty tokens created by consecutive/trailing tabs
  nz <- which(nzchar(tokens))
  if (!length(nz)) return(NA_integer_)
  toks <- tokens[nz]
  # scan from end for an integer-looking token (allow leading -)
...
count_mutations_in_file <- function(file) {
  ...
  for (i in seq.int(1L, length(lines), by = 2L)) {
    ...
    # Trim whitespace and remove any CR/LF remnants
    toks <- trimws(toks, which = "both")

    # Find generation as the last non-empty integer token
...
if (!is.null(mut_count_df) && nrow(mut_count_df)) {
  mut_count_df <- mut_count_df[order(mut_count_df$n, mut_count_df$psi,
                                     mut_count_df$shift_size, mut_count_df$rep,
...
plot_mut_counts_over_psi <- function(mut_count_df,
  ...
  d0 <- subset(mut_count_df, generation == gen_target)
  if (nrow(d0) == 0) {
    warning("No rows at generation ", gen_target, " in mut_count_df.")
    return(invisible(NULL))
  }

  for (n_val in sort(unique(d0$n))) {
    d_n <- subset(d0, n == n_val)
    if (nrow(d_n) == 0) next
    ...
    for (sh in sort(unique(d_n$shift_size))) {
      ...
      # --- NEW: nicer labels for shift_size == 0 ---
      title_shift <- if (is.finite(sh) && sh == 0) "data-driven" else paste0("shift_size = ", sh)
      file_shift  <- if (is.finite(sh) && sh == 0) "dataDriven"  else paste0("shift_", sh)

      fname <- paste0("mut_count_over_psi_n_", n_val, "_", file_shift, "_gen", gen_target, ".png")
      png(file.path(out_dir, fname), width = 1000, height = 700)
      par(mar = c(4.5, 4.5, 3.5, 1) + 0.1)
      ...
      plot(NA,
           ...
           main = paste0("Mutations vs psi | n = ", n_val,
                         " | ", title_shift,
                         " | gen = ", gen_target),
           xaxt = "n")
...
# Assuming mut_count_df is built as before:
plot_mut_counts_over_psi(mut_count_df, gen_target = 10000L,
                         out_subfolder = "mut_counts_by_psi")
...
# --- Filename metadata: mut/n_{n}_psi_{psi}_sz_{shift}_{rep}.tsv ---
extract_mut_metadata <- function(path) {
  rx <- "mut/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_([0-9]+)\\.tsv"
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
...
# Columns: n, psi, shift_size, rep, file_id, generation, bin_mid, prop
mut_hist_long <- (function() {
  ...
  for (file in mut_files) {
    ...
    for (i in seq.int(1L, length(lines), by = 2L)) {
      ...
      # fallback: if even row malformed, try IDs row length (still not ideal, but safer than dropping)
...
# Gives one row per (n, psi, shift_size, generation, bin_mid)
if (nrow(mut_hist_long)) {
  # mean prop per group/bin
  mut_hist_mean <- aggregate(prop ~ n + psi + shift_size + generation + bin_mid,
                             data = mut_hist_long, FUN = function(x) mean(x, na.rm = TRUE))
  # (optional) how many reps contributed to each mean
  mut_hist_n <- aggregate(prop ~ n + psi + shift_size + generation + bin_mid,
                          data = mut_hist_long, FUN = function(x) sum(is.finite(x)))
  names(mut_hist_n)[names(mut_hist_n) == "prop"] <- "n_reps"
  mut_hist_mean <- merge(mut_hist_mean, mut_hist_n,
                         by = c("n","psi","shift_size","generation","bin_mid"),
...
plot_mut_hist_two_gens <- function(mut_hist_mean,
  ...
  for (n_val in sort(unique(mut_hist_mean$n))) {
    df_n <- subset(mut_hist_mean, n == n_val)
    if (nrow(df_n) == 0) next
    ...
    for (sh in sort(unique(df_n$shift_size))) {
      ...
      for (g in gens) {
        ...
        if (length(mids_all) > 1) {
          ...
          xlim <- range(mids_all)  # fallback
        }

        # File + device
        fname <- paste0("mut_avg_hist_n_", n_val, "_", file_shift, "_gen", g, ".png")
        png(file.path(out_dir, fname), width = 1000, height = 700)
        par(mar = c(4.5, 4.5, 3.5, 1) + 0.1)
...
```{r}
plot_mut_hist_two_gens(mut_hist_mean, gens = c(10000, 10005),
                       out_subfolder = "mut_effect_hist_avg_two_gens")
```
