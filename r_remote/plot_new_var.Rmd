...
base_plot_folder <- "../r_plots/new_variance"
target_psi <- c(0.0, 0.05, 0.3, 0.5, 0.7, 0.9, 1.0)
...
# Matches: genotype/n_<n>_psi_<psi>_sz_<sz>_reg_<inv|var>_<rep>.tsv
extract_geno_metadata <- function(path) {
  pat <- "genotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|var)_(\\d+)\\.tsv"
...
# Matches: phenotype/n_<n>_psi_<psi>_sz_<sz>_reg_<inv|var>_<rep>.tsv
extract_pheno_metadata <- function(path) {
  m <- regexec("phenotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|var)_(\\d+)\\.tsv", path)
...
#   data/n_<n>/psi_<psi>/sz_<shift_size>/_reg_<inv|var>_<rep>.tsv
extract_sum_metadata <- function(path) {
  rx <- "data/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|var|ext)_(\\d+)\\.tsv"
  m  <- regexec(rx, path)
  ...
  for (reg_lab in regimes) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in shift_keys) {
        ...
        out_name <- paste0("summary_multi_by_psi_ticks_reg_", reg_lab,
        ...
        mtext(paste0("Summary: dist_norm * Va0_sqrt over ticks (9999 uses |z_mean - opt@10000|) | reg = ",
  ...
  for (reg_val in sort(unique(df$reg))) {
    ...
    for (n_val in sort(unique(df_reg$n))) {
      ...
      for (sk in sort(unique(df_n$shift_size))) {
        ...
        for (psi_val in sort(unique(df_sk$psi))) {
          df_p <- subset(df_sk, psi == psi_val & tick %in% 9999:10000)
          if (!nrow(df_p)) next
...
```{r}
# Plot fitted decay metric (k or half-life) vs psi, grouped by shift_key == "shift=..."
# Expects columns: reg, n, shift_key, psi, and either k or t_half
plot_decay_metric_by_shiftkey <- function(decay_tbl,
                                          metric = c("k","t_half")[1],
                                          out_subfolder = "decay_by_shiftkey",
                                          log_y = FALSE) {
  metric <- match.arg(metric)
  stopifnot(all(c("reg","n","shift_key","psi",metric) %in% names(decay_tbl)))

  # Keep finite metric rows
  df <- decay_tbl[is.finite(decay_tbl[[metric]]), , drop = FALSE]
  ...
  for (sk in sort(unique(df$shift_size))) {
    ...
    for (n_val in ns) {
      ...
      if (log_y) {
        ...
        plot(NA, xlim = range(psis), ylim = yl, log = "y",
             xlab = "psi", ylab = ylab,
             main = paste0("n = ", n_val, "  |  shift = ", sk))
      } else {
        plot(NA, xlim = range(psis), ylim = ylim,
             xlab = "psi", ylab = ylab,
             main = paste0("n = ", n_val, "  |  shift = ", sk))
      }

      # One line per reg across psi
      for (rg in regs) {
        dpr <- dp[dp$reg == rg, , drop = FALSE]
        if (!nrow(dpr)) next
        ord <- order(dpr$psi)
        x <- dpr$psi[ord]; y <- dpr[[metric]][ord]
    ...
    mtext(paste0("Fitted exponential decay (", metric, ") vs psi  |  shift = ", sk),
          outer = TRUE, cex = 1.1, line = 1)
...
plot_decay_heatmap_per_shift <- function(decay_tbl,
  ...
  metric <- match.arg(metric)
  need <- c("reg","n","psi","shift_key", metric)
  ...
  for (sh in sort(unique(df$shift_size))) {
    ...
    for (idx in seq_len(nrow*ncol)) {
      if (idx <= R) {
        ...
        # build psi × n matrix (mean if duplicates)
        M <- matrix(NA_real_, nrow = length(psis_all), ncol = length(ns_all),
                    dimnames = list(paste0("psi=", psis_all), paste0("n=", ns_all)))
        if (nrow(dpr)) {
          for (i in seq_along(psis_all)) {
            for (j in seq_along(ns_all)) {
              v <- dpr[dpr$psi == psis_all[i] & dpr$n == ns_all[j], metric, drop = TRUE]
              if (length(v)) M[i, j] <- mean(v, na.rm = TRUE)
            }
          }
        }

        image(x = seq_along(ns_all), y = seq_along(psis_all), z = t(M),
              xaxt = "n", yaxt = "n", xlab = "n", ylab = "psi",
              col = cols, zlim = zlim)
        axis(1, at = seq_along(ns_all), labels = ns_all)
        axis(2, at = seq_along(psis_all), labels = psis_all, las = 1)
        box()
...
plot_decay_heatmap_per_shift(decay_tbl, metric = "k", out_subfolder = "decay_k")
```
...
plot_jump_heatmap <- function(jump_tbl,
  ...
  need <- c("reg","n","psi","shift_size","jump")
  ...
  # Use common psi/n grids so panels align
  psis_all <- sort(unique(df$psi))
  ns_all   <- sort(unique(df$n))
  ...
  for (sh in sort(unique(df$shift_size))) {
    ...
    for (idx in seq_len(nrow*ncol)) {
      if (idx <= R) {
        ...
        # Build psi × n matrix; average if duplicate rows exist
        M <- matrix(NA_real_, nrow = length(psis_all), ncol = length(ns_all),
                    dimnames = list(paste0("psi=", psis_all), paste0("n=", ns_all)))
        if (nrow(dpr)) {
          for (i in seq_along(psis_all)) {
            for (j in seq_along(ns_all)) {
              v <- dpr[dpr$psi == psis_all[i] & dpr$n == ns_all[j], "jump", drop = TRUE]
              if (length(v)) M[i, j] <- mean(v, na.rm = TRUE)
            }
          }
        }
...
# Filename: ind/n_{n}_psi_{psi}_sz_{shift}_reg_{inv|var}_{rep}_gen{gen_filter}.tsv
extract_effect_metadata <- function(path) {
  rx <- paste0("ind/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|var)_(\\d+)_gen", gen_filter, "\\.tsv")
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 6) {
...
#   mut/n_{n}_psi_{psi}_sz_{shift}_reg_{inv|var}_{rep}.tsv
extract_mut_metadata <- function(path) {
  rx <- "mut/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|var)_(\\d+)\\.tsv"
  m  <- regexec(rx, path)
  p  <- regmatches(path, m)[[1]]
  if (length(p) != 6) {
    warning("Could not extract metadata from path: ", path, " | parts: ", paste(p, collapse=", "))
