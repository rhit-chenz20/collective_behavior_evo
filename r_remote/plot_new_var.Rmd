...
base_data_folder <- "../data/new_variance_ext"
base_plot_folder <- "../r_plots/new_var_plots2"
target_beta <- c(0.0, 0.01, 0.03, 0.05, 0.07, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0)
regimes <- c("inv")  # "inv" = inverse_variance, "var" = variance
```
...
# -------- Load phenotype_df (no name collisions with vars) --------
opt_gen <- 10001L
...
# ---- same preamble (tick_window, colors, .compute_seg_series) ----
...
plot_seg_site_by_tick <- function(df,
  ...
  df$shift_size <- suppressWarnings(as.numeric(df$shift_size))  # <--- NEW: ensure present/numeric
  ...
  for (reg_lab in regimes) {
    ...
    out_dir_reg <- file.path(base_plot_folder, out_root, paste0("reg_", reg_lab))
    if (!dir.exists(out_dir_reg)) dir.create(out_dir_reg, recursive = TRUE)
    ...
    # ---- NEW: split by shift_size first ----
    shift_vals <- sort(unique(df_reg$shift_size))
    shift_vals <- shift_vals[is.finite(shift_vals)]
    if (!length(shift_vals)) {
      warning("No finite shift_size values found for reg=", reg_lab)
      next
    }

    for (sh in shift_vals) {
      df_sh <- subset(df_reg, is.finite(shift_size) & shift_size == sh)
      if (!nrow(df_sh)) next

      # Optional: put each shift into its own subfolder for tidiness
      sh_tag <- gsub("[^A-Za-z0-9._-]+", "_", formatC(sh, format = "fg", digits = 8))
      out_dir <- file.path(out_dir_reg, paste0("shift_", sh_tag))
      if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

      # Unique split values (psi / beta / dist_norm)
      split_vals <- sort(unique(df_sh[[split_var]]))

      # Panels are by n
      n_vals <- sort(unique(df_sh$n))
      ...
      # ---- inner loop: one PDF per (shift_size, split_var) ----
      for (sv in split_vals) {
        df_sv <- subset(df_sh, df_sh[[split_var]] == sv)
        if (!nrow(df_sv)) next
        ...
        # Top title per PDF
        mtext(paste0("seg_site vs tick (", min(tick_window), ":", max(tick_window), ")",
                     " | reg = ", reg_lab,
                     " | shift_size = ", sh,
...
compute_jump <- function(df, tick_seq = 10099:10100, relative=TRUE) {
...
```
unique(mut_count_df$generation)
      ...
      for (sh in sort(unique(d_n$shift_size))) {
        ...
        plot(NA,
             ...
             main = paste0("Mutations vs beta | reg = ", reg_lab,
                           ...
                           " | gen = ", gen_target),
             xaxt = "n")
        axis(1, at = beta_grid, labels = formatC(beta_grid, format = "f", digits = 2))
...
# load group variance data (not working)
```{r}
# Set the top-level folder
folder <- paste0(base_data_folder, "/vars")
...
# Function to extract metadata from path
# Matches: vars/n_<n>_beta_<beta>_sz_<sz>_reg_<inv|var>_<rep>.tsv
extract_geno_metadata <- function(path) {
  pat <- "vars/n_(\\d+)_beta_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|var)_(\\d+)\\.tsv"
  matches <- regexec(pat, path)
  parts <- regmatches(path, matches)[[1]]
...
# Helper: get shift_size from the companion "data" file (optimum at generation == 10001)
get_shift_from_data <- function(geno_file, base_data_folder, opt_gen = 10001L) {
  # Mirror the path: .../vars/... -> .../data/...
  data_path <- sub("/vars/", "/data/", geno_file, fixed = TRUE)
  if (!file.exists(data_path)) {
    data_path <- file.path(base_data_folder, "data", basename(geno_file))
  }
...
# Read and combine all files
opt_gen <- 10001L
vars_df <- do.call(rbind, lapply(file_list, function(file) {
  df <- tryCatch(read.table(file, header = TRUE, sep = "\t"), error = function(e) NULL)
  if (is.null(df)) {
