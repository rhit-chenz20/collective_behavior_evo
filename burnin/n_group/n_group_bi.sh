...
bin/slim5.0 -d psi=$psi -d n=$n -d ID="${SLURM_JOB_ID}" -d "folder='/workdir/$USER/${SLURM_ARRAY_JOB_ID}-${SLURM_ARRAY_TASK_ID}/pop'" n_group_bi.slim &> /workdir/$USER/${SLURM_ARRAY_JOB_ID}-${SLURM_ARRAY_TASK_ID}/log/rec_psi_${psi}_n_${n}log_${SLURM_JOB_ID}.txt 
