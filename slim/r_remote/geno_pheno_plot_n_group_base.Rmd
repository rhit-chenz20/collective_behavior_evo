...
folder <- base_data_folder + "/genotype"
...
for (n_val in sort(unique(genotype_df$n))) {
  ...
  burnin_ids <- unique(df_n$burnin_id)
  ...
  for (bid in burnin_ids) {
    ...
    plot(NULL, xlim = c(-750, 750), ylim = c(0, 1.0),
         xlab = "Genotype Value", ylab = "Proportion",
         main = paste("Avg Genotype Histogram | n =", n_val, "| burnin_id =", bid))
    ...
    for (psi in psi_in_this_burnin) {
      ...
      for (r in seq_along(reps)) {
        ...
        genos <- genos[!is.na(genos)]
        h <- hist(genos, breaks = breaks, plot = FALSE)
        counts_mat[r, ] <- h$counts / sum(h$counts)
      }
