...
setwd(wd_path)
# base_data_folders <- c("../../data/fluctuate_env_constant_psi_gap_1", "../../data/fluctuate_env_constant_psi_gap_10", "../../data/fluctuate_env_constant_psi_gap_100")
base_data_folders <- c("../../data/fluctuate_env_constant_psi_gap_100")
...
extract_sum_metadata <- function(path) {
    rx <- "fluctuate_env_constant_psi_gap_(\\d+)/data/n_(\\d+)_(psi|beta)_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|fit|ext|ave)_(\\d+)\\.tsv"
    m  <- regexec(rx, path)
    ...
    data.frame(
        ...
        n          = as.integer(p[3]),
        var        = p[4],
        value      = as.numeric(p[5]),
        shift_size = as.integer(p[6]),
        reg        = p[7],               
        rep        = as.integer(p[8]),
...
```{r}
# Save
saveRDS(summary_stat_df, file = "../r_data/fluctuate_env_constant_psi/data_summary.rds")
```
...
extract_geno_metadata <- function(path) {
  pat <- "fluctuate_env_constant_psi_gap_(\\d+)/genotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(fit|ext|ave)_(\\d+)\\.tsv$"
  ...
  data.frame(
    fluctuate_gap = as.integer(parts[2]),
...
dfs <- lapply(folders, function(folder) {
  ...
  file_list <- file_list[!grepl("_reg_inv_", file_list)]
...
```{r}
saveRDS(genotype_df, file = "../r_data/fluctuate_env_constant_psi/genotypes_gap100.rds")
```
...
extract_pheno_metadata <- function(path) {
  pat <- "fluctuate_env_constant_psi_gap_(\\d+)/phenotype/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(fit|ext|ave)_(\\d+)\\.tsv$"
  matches <- regexec(pat, path)
  parts <- regmatches(path, matches)[[1]]
...
```{r}
saveRDS(phenotype_df, file = "../r_data/fluctuate_env_constant_psi/phenotype_gap100.rds")
```
