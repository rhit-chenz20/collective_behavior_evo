...
setwd(wd_path)
base_data_folders <- c("../../data/e_psi_from_constant_to_variation")
rds_data_folder <- "../r_data/e_psi_from_constant_to_variation"
base_plot_folder <- "plots"
target_psi <- c(0.2, 0.5, 0.8)
...
extract_sum_metadata <- function(path) {
    rx <- "data/n_(\\d+)_psi_(-?[0-9.]+)_sz_(\\d+)_reg_(inv|fit|ext|ave)_(\\d+)\\.tsv"
    m  <- regexec(rx, path)
...
```{r}
# Save
saveRDS(summary_stat_df, file = paste0(rds_data_folder, "/summary.rds"))
```

```{r}
# Later (or on another machine)
summary_stat_df <- readRDS("../r_data/e_psi_from_constant_to_variation/summary.rds")
```
...
```{r}
saveRDS(genotype_df, file = paste0(rds_data_folder, "/genotypes.rds"))
```
...
```{r}
saveRDS(phenotype_df, file = paste0(rds_data_folder, "/phenotypes.rds"))
regimes <- c("fit", "ave", "ext", "inv")
```
...
```{r}
saveRDS(psis_betas_df, file = paste0(rds_data_folder, "/psis.rds"))
```
...
```{r}
saveRDS(fitness_df, file = paste0(rds_data_folder, "/fitnesses.rds"))
```
